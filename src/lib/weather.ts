export async function getWeatherHumidity(
  lat: string,
  lon: string
): Promise<number> {
  ...
  if (hourlyData.hourly?.relative_humidity_2m && hourlyData.hourly.time) {
    ...
    let idx = times.findIndex(
      (t: Date) => Math.abs(t.getTime() - now.getTime()) < 60 * 60 * 1000
    );
    if (idx === -1) idx = 0;
    return Math.round(hourlyData.hourly.relative_humidity_2m[idx]);
  }

  throw new Error('Erro ao obter umidade. Usando valor padrão.');
}
